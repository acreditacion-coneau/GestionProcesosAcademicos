...
export function ResponsableCatedraDashboard() {
  const { user } = useUser();
  ...
  useEffect(() => {
    ...
    getAsignacionesEvaluador(user.idDocente, "responsable_catedra").then((data) => {
      setPendientesEval(data.filter((a) => a.estado === "pendiente").length);
    });
  }, [user.idDocente]);
  ...
  const menuData = [
    ...
    },
    {
      ...
      headerColor: "bg-[#fce7d6] text-[#7c3a1e]",
      iconColor: "text-[#7c3a1e]",
        ...
        <div className="bg-white p-8 rounded-2xl shadow-sm border border-slate-100 flex justify-between items-center mb-8">
          <div>
            ...
            <p className="text-slate-500 mt-2 text-lg font-light">
              {user.carrera} - Gestione las evaluaciones de su equipo docente.
            </p>
          </div>
        </div>
        ...
        <div className="space-y-6">
          ...
          {(activeFilter === "all" || activeFilter === "evaluacion_docente") && (
            <div className="bg-white rounded-2xl shadow-sm overflow-hidden border border-slate-100">
              ...
              >
                <h3 className="text-base font-bold tracking-wide flex items-center gap-2">
                  {menuData[2].icon} Evaluación Docente
                </h3>
                {openCards.evaluacion_docente ? <ChevronUp className="w-5 h-5" /> : <ChevronDown className="w-5 h-5" />}
              </div>

              {openCards.evaluacion_docente && (
                <div className="p-6 flex flex-col sm:flex-row items-center justify-between gap-6 hover:bg-slate-50/50 transition-colors">
                  <div>
                    <h4 className="font-semibold text-slate-800 text-lg mb-1">Evaluaciones pendientes de su carrera</h4>
                    <p className="text-sm text-slate-500">Revise y complete las evaluaciones docentes asignadas a su rol.</p>
                    {pendientesEval !== null && pendientesEval > 0 && (
                      <p className="text-sm font-semibold text-amber-600 mt-1">
                        {pendientesEval} evaluación{pendientesEval !== 1 ? "es" : ""} pendiente{pendientesEval !== 1 ? "s" : ""}
                      </p>
                    )}
                  </div>
